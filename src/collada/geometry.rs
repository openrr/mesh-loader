...
pub struct LibraryGeometries {
    ...
    pub geometries: IndexMap<String, Geometry>,

    pub accessors: HashMap<String, Accessor>,
    pub array_data: HashMap<String, ArrayData>,
}
...
#[derive(Debug)]
#[non_exhaustive]
pub struct VerticesInputs {
    pub position: UnsharedInput,
    pub normal: Option<UnsharedInput>,
    pub texcoord: Option<UnsharedInput>,
}
...
pub struct PrimitiveInputs {
    pub vertex: SharedInput<Vertices>,
    pub normal: Option<SharedInput>,
    pub color: Option<SharedInput>,
    pub texcoord: Vec<SharedInput>,
}
...
pub struct Primitive {
    ...
    pub input: Option<PrimitiveInputs>,
...
pub(crate) fn parse_library_geometries(cx: &mut Context, node: xml::Node<'_, '_>) -> Result<()> {
    ...
    if cx.library_geometries.geometries.is_empty() {
        error::one_or_more_elems(node, "geometry")?;
    }

    Ok(())
}

fn parse_geometry(cx: &mut Context, node: xml::Node<'_, '_>) -> Result<Option<Geometry>> {
...
fn parse_mesh(cx: &mut Context, node: xml::Node<'_, '_>) -> Result<Mesh> {
    ...
    if !has_source {
        error::one_or_more_elems(node, "source")?;
    }
...
fn parse_vertices(node: xml::Node<'_, '_>) -> Result<Vertices> {
    ...
    let input_position = match input_position {
        ...
        None => error::one_or_more_elems(node, "input")?,
    };
...
fn parse_primitive(node: xml::Node<'_, '_>, ty: PrimitiveType) -> Result<Primitive> {
        ...
        match node.tag_name().name() {
            ...
            "vcount" => {
                ...
                if ty != PrimitiveType::Polylist {
                    error::unexpected_child_elem(node)?;
                }
                if !vcount.is_empty() {
                    error::multiple_elems(node)?;
                }
