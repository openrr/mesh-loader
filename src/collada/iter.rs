...
impl<'a> Primitive<'a> {
    ...
    pub fn texcoords(&self, set: usize) -> Texcoords<'a> {
        let acc = match &self.xml.input {
            Some(input) => {
    ...
    pub fn vertex_indices(&self) -> VertexIndices<'a> {
    ...
    pub fn normal_indices(&self) -> VertexIndices<'a> {
        let offset = match &self.xml.input {
            Some(input) => match &input.normal {
    ...
    pub fn texcoord_indices(&self, set: usize) -> VertexIndices<'a> {
        let offset = match &self.xml.input {
...
#[derive(Debug)]
pub struct Positions<'a>(Option<(u32, slice::Chunks<'a, f32>)>);
...
pub type Normals<'a> = Positions<'a>;

#[derive(Debug)]
pub struct Texcoords<'a>(Option<TexcoordsInner<'a>>);
